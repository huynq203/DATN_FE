...
export default function ManagerRole() {
  ...
  const [roleDetail, setRoleDetail] = useState<DataType>()
  const {
    data: roleData,
    isLoading,
    refetch
  ...
  const listRole = roleData?.data.result
  ...
  return (
    <div className=''>
      <Helmet>
        <title>Quản lý vai trò - YOYO Store</title>
      ...
      <div className='rounded-md grid grid-cols-2 bg-gray-50 p-2 m-2'>
        <div className='flex col-span-1 text-lg font-bold capitalize mt-1'>Quản lý vai trò</div>
        <div className='flex col-span-1 justify-end'>
          ...
          >
            <div className='flex'>
              ...
              <span className=''>Thêm vai trò</span>
            </div>
          </Button>
        </div>
      </div>
      ...
      <ModalCreateRole isModalOpen={isOpenModalCreate} setIsModalOpen={setIsOpenModalCreate} />
      <ModalUpdateRole
        isModalOpen={isOpenModalUpdate}
        setIsModalOpen={setIsOpenModalUpdate}
        roleDetail={roleDetail as DataType}
      />
    </div>
  )
}
