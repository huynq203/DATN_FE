...
class CartApi {
  addToCart(body: { product_id: string; size: number; color: string; quantity: number }) {
  ...
  updateCart(body: { product_id: string; size: number; color: string; quantity: number }) {
    return http.put<SuccessResponseApi<Cart[]>>(paths.ApiPath.CART_UPDATE_PRODUCT, body)
  }
  deleteCart(body: { product_id: string; size: number; color: string }[]) {
